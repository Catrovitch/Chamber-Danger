...
class Node:
    ...
    def __init__(self, x, y, width, height, node_nr):
        """This class functions as the nodes in the graph generated by the BSP algorithm.
        """
        self.x = x
        self.y = y
        self.width = width
    ...
    def splitNode(self):
        ...
        if (horizontal_split):
            self.child_1 = Node(self.x, self.y, self.width,
                                split, self.node_nr+1)
            self.child_2 = Node(self.x, self.y+split,
                                self.width, self.height-split, self.node_nr+2)
        else:
            self.child_1 = Node(self.x, self.y, split,
                                self.height, self.node_nr+1)
